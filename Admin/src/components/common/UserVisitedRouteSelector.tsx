...
import { userVisitedRoutesService, AvailableRoute } from '../../services/userVisitedRoutes.service';

interface UserVisitedRouteSelectorProps {
  userId: number;
  value: number | null;
...
const UserVisitedRouteSelector: React.FC<UserVisitedRouteSelectorProps> = ({
  userId,
  value,
  onChange,
  ...
  const loadAvailableRoutes = async () => {
    setLoading(true);
    try {
      const data = await userVisitedRoutesService.getAvailableRoutes(userId);
      setRoutes(data || []);
    } catch (error) {
...
export default UserVisitedRouteSelector;
