...
import { FiArrowLeft, FiTrash2, FiPlus, FiMapPin, FiMusic, FiPlay, FiStar, FiImage, FiEdit2, FiCheck, FiX } from 'react-icons/fi';
import { toast } from 'react-toastify';
import locationsService, { Location, LocationType, StoryLocation } from '../../services/locations.service';
  ...
  const loadLocation = async () => {
    try {
      ...
      setEditForm({
        ...
        addressTextRefId: data.addressTextRefId || '',
  ...
  const handleSaveEdit = async () => {
    try {
      await locationsService.updateLocation(Number(id), {
        ...
        addressTextRefId: editForm.addressTextRefId || undefined,
        locationType: editForm.locationType as LocationType,
    ...
    <div>
      ...
      <div className="mb-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-white mb-2">Location Details</h1>
            <p className="text-gray-400">Manage location information and stories</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            {editMode ? (
              ...
              <p className="text-white">{location.nameTextRefId}</p>
            )}
          </div>
          <div>
          ...
          <div>
            <label className="text-gray-400 text-sm">Location Type</label>
            {editMode ? (
              <select
                value={editForm.locationType}
                onChange={(e) => setEditForm({ ...editForm, locationType: e.target.value as LocationType | '' })}
                className="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-white focus:outline-none focus:border-blue-500"
              >
              ...
              <p className="text-white">{location.locationType || 'N/A'}</p>
            )}
          </div>
          <div>
          ...
          <div>
            ...
            {editMode ? (
              ...
              <p className="text-white">{location.longitude}</p>
            )}
          </div>
          <div>
            <label className="text-gray-400 text-sm">Address Text Ref ID</label>
            {editMode ? (
              <input
                type="text"
                value={editForm.addressTextRefId}
                onChange={(e) => setEditForm({ ...editForm, addressTextRefId: e.target.value })}
                className="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-white focus:outline-none focus:border-blue-500"
              />
            ) : (
              <p className="text-white">{location.addressTextRefId || 'N/A'}</p>
            )}
          </div>
          <div>
            ...
            {editMode ? (
              ...
              <p className="text-white flex items-center gap-2">
                ...
                {location.rating || 'No rating'}
              </p>
            )}
          </div>
          <div>
            <label className="text-gray-400 text-sm">Photos</label>
            <p className="text-white flex items-center gap-2">
              <FiImage className="text-gray-400" />
              {location.photoUrls?.length || 0} photos
            </p>
          </div>
          <div>
          ...
          <div className="md:col-span-2">
            ...
            <div className="mt-2 flex items-center gap-2">
              ...
              <span className="text-white">
                {location.photoUrls.length} photo{location.photoUrls.length !== 1 ? 's' : ''}
              </span>
            </div>
          </div>
        )}
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-gray-900 rounded-xl border border-gray-800 p-6 max-w-md w-full mx-4">
            <h3 className="text-xl font-semibold text-white mb-4">Delete Location</h3>
